...
class Graph(object):
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        """
        simple = self._simple and other._simple
        directed = self._directed and other._directed
        union = Graph(directed, 0, simple)
        v_map = union.copy_vertices(self._v + other._v)

        for e in self._e + other._e:
            e_copy = Edge(v_map[e.tail], v_map[e.head], e.weight)
            union.add_edge(e_copy)
    ...
    def copy_vertices(self, vertices: List["Vertex"]):
        v_map = {}
        for v in vertices:
            v_copy = Vertex(self)
            self.add_vertex(v_copy)
            v_map[v] = v_copy
        return v_map

    def deep_copy(self) -> "Graph":
        graph = Graph(self._directed, 0, self._simple)
        v_map = graph.copy_vertices(self._v)

        for e in self._e:
            e_copy = Edge(v_map[e.tail], v_map[e.head])
    ...
    def complement(self) -> "Graph":
        ...
        for i in range(v_len):
            for j in range(0 if self._directed else i + 1, v_len):
                v1 = self._v[i]
                v2 = self._v[j]
                if v1 != v2 and not v1.is_adjacent(v2):
                    edge = Edge(v_map[v1], v_map[v2])
                    graph.add_edge(edge)

        return graph

    def del_edge(self, edge: "Edge"):
        edge._head._incidence.pop(edge._tail)
        edge._tail._incidence.pop(edge._head)
        self._e.remove(edge)
    ...
    def is_tree(self):
        """
        It is a tree, if it is a forest and |E| = |V| - 1
        """
        return self.is_forest() and len(self._e) == len(self._v) - 1

    def is_forest(self):
        """
        It is a forest, if |E| > |V| - 1 and there are no cycles
        """
        if len(self._e) > len(self._v) - 1:
            return False

        # assign indices to each vertex
        self.indices = {v: i for i, v in enumerate(self._v)}

        # and check if there are any cycles
        visited = [False] * len(self._v)
        return not self.is_cyclic(0, -1, visited)

    def is_cyclic(self, v_idx, parent_idx, visited):
        """
        Recursively checks if there are any cycles.
        """
        visited[v_idx] = True
        for n in self._v[v_idx].neighbours:
            n_idx = self.indices[n]
            if visited[n_idx] == False:
                if self.is_cyclic(n_idx, v_idx, visited) == True:
                    return True
            elif n_idx != parent_idx:
                return True
        return False
